...
get_soft(){
    #软件下载
    if [ ! -e mysql-5.7.9-linux-glibc2.5-x86_64.tar.gz ]; then
        echo "开始下载mysql-5.5.46-2.tar"
        wget http://oms.2144gy.com:810/upfiles/soft/mysql-5.7.9-linux-glibc2.5-x86_64.tar.gz
        echo "软件下载成功，开始解压配置"
    fi
}
...
Insall(){
    ...
    echo "解压已完成。"
    mv mysql-5.7.9-linux-glibc2.5-x86_64/* ${1}
    echo "目录移动已完成##########################"
...
import_config(){
    ...
    echo "开始导入my.cnf############################"
    wget -P ./mysql57_tmp/ http://oms.2144gy.com:810/upfiles/soft/config/mysql57/my${1}.cnf
    wget -P ./mysql57_tmp/ http://oms.2144gy.com:810/upfiles/soft/config/mysql57/mysqld${1}
    dos2unix ./mysql57_tmp/mysqld${1}
    cp -rf ./mysql57_tmp/my${1}.cnf ${2}/my.cnf
    cp -rf ./mysql57_tmp/mysqld${1} /etc/init.d/
    rm -rf ./mysql57_tmp/
    chmod 755 /etc/init.d/mysqld${1}
    chkconfig --add mysqld${1}
    echo "mysqld${1}加入开机启动完成"
...
db_config(){
    ...
    chown -R mysql.mysql ${2}
    ${1}/bin/mysqld --initialize-insecure  --basedir=$1 --datadir=${2}/data/ --user=mysql
    echo "数据库初始化已完成!#########################"
}
